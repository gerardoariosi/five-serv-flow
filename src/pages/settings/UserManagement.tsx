...
const UserManagement = () => {
  ...
  const [editDialog, setEditDialog] = useState<{ open: boolean; user: UserRow | null; mode: 'create' | 'edit' }>({ open: false, user: null, mode: 'create' });
    ...
    <div className="p-4 max-w-4xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <Select value={roleFilter} onValueChange={setRoleFilter}>
          <SelectTrigger className="w-40 bg-secondary border-border"><SelectValue placeholder="Role" /></SelectTrigger>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    {user.last_active_at && (
                      <span className="text-xs text-muted-foreground">Last active: {format(new Date(user.last_active_at), 'MMM d, HH:mm')}</span>
                    )}
                  </div>
                </div>
                <div className="flex gap-1 flex-shrink-0">
                  ...
                  <button onClick={() => toast.info('Reset password email would be sent to ' + user.email)} className="p-2 text-muted-foreground hover:text-foreground" title="Reset Password"><KeyRound className="w-4 h-4" /></button>
                  <button onClick={() => forceResetMutation.mutate(user.id)} className="p-2 text-muted-foreground hover:text-foreground" title="Force Password Reset"><Lock className="w-4 h-4" /></button>
                  <button onClick={() => toggleNotificationsMutation.mutate({ userId: user.id, enabled: !user.notifications_enabled })} className={`p-2 ${user.notifications_enabled ? 'text-muted-foreground' : 'text-destructive'} hover:text-foreground`} title={user.notifications_enabled ? 'Disable Notifications' : 'Enable Notifications'}>
        ...
        <DialogContent className="max-w-md">
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {roleOptions.map(role => (
                  <button key={role} onClick={() => toggleRole(role)}
                    className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors capitalize ${form.roles.includes(role) ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}>
                    {role}
                  </button>
                ))}
              </div>
          ...
          <DialogFooter>
            <Button onClick={() => editDialog.mode === 'create' ? createUserMutation.mutate() : updateUserMutation.mutate()}
              disabled={!form.full_name || !form.email || createUserMutation.isPending || updateUserMutation.isPending}
