...
const VendorForm = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) {
        const { data, error } = await supabase.from('technicians_vendors').insert(payload).select('id').single();
        if (error) throw error;
        toast.success('Vendor created');
        navigate(`/team/vendors/${data.id}`);
      } else {
        const { error } = await supabase.from('technicians_vendors').update(payload).eq('id', id!);
        if (error) throw error;
        toast.success('Vendor updated');
        navigate(`/team/vendors/${id}`);
      }
    } catch (e: any) {
  ...
  const handleBack = () => {
    if (isNew) navigate('/team/technicians');
    else navigate(`/team/vendors/${id}`);
  };
